...
- **Notifications stay quiet.** One banner per port per minute, one sound per burst, and a new banner about a port replaces the old one. While the screen is locked or the display is asleep nothing is posted at all: the changes are counted and one summary arrives when you come back, so an agent restarting a server for an hour cannot queue an hour of alerts.
- **Watch and guard.** Every row carries a shield: guard a port and whatever takes it is stopped for you (it asks first, and never touches a running agent's server, a protected process, or a system port). The shield turns orange while a guard is on. The star, on hover or keyboard selection, watches a port instead: watched ports sit above the list when you have not typed or filtered, with live status including "free", and tell you when they change. A guard that fires repeatedly stands itself down rather than fighting a supervisor.
