...
- **Watch and guard.** Every row has a star and a shield on hover or keyboard selection: watch a port to be told when it frees up or gets taken, guard it to have whatever takes it stopped for you (it asks first, and never touches a running agent's server, a protected process, or a system port). A guarded port wears an orange shield; watched ports sit above the list when you have not typed or filtered, with live status including "free". A guard that fires repeatedly stands itself down rather than fighting a supervisor.
